...
public class BoardPagination {
	...
	public static String pagingStrComm(int totalCount, int pageSize, int blockPage,
			int pageNum, String reqURL, String boardIdx) {
		...
		// pageTemp가 1(첫번째 페이지 블록)이 아닐때만 이전블럭 바로가기 링크 출력
		if(pageTemp != 1) {
			pagingStr.append("<a href='").append(reqURL).append("?idx=" + boardIdx).append("&pageNum=1");
			pagingStr.append("'> [첫 페이지]</a>");
			pagingStr.append("&nbsp;");
			pagingStr.append("<a href='").append(reqURL).append("?idx=" + boardIdx).append("&pageNum=").append(pageTemp - 1);
			pagingStr.append("'> [이전 블럭]</a>");
		}
		...
		while (blockCount <= blockPage && pageTemp <= totalPages) {
			if (pageTemp == pageNum) {
				...
				// 현재 페이지가 아닌 경우에만 링크를 추가한다.
				pagingStr.append("&nbsp;<a href='").append(reqURL).append("?idx=" + boardIdx).append("&pageNum=").append(pageTemp);
				pagingStr.append("'>").append(pageTemp).append("</a>&nbsp;");
			}
			// 반복하면서 1씩 증가시켜 순차적인 페이지번호를 출력한다.
			pageTemp++;
		...
		if (pageTemp <= totalPages) {
			...
			pagingStr.append("'>[다음 블럭]</a>");
			pagingStr.append("&nbsp;");
			pagingStr.append("<a href='").append(reqURL).append("?idx=" + boardIdx).append("&pageNum=").append(totalPages);
			pagingStr.append("'>[마지막 페이지]</a>");
		}
		return pagingStr.toString();
	}
