...
public class QnABoardListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 게시물 개수 카운트
		int totalCount = dao.selectCountQnA(map);
		...
		List<BoardDTO> boardLists = dao.selectListPageQnA(map);
		dao.close();
		
		// 페이지 번호를 String 타입으로 변환
		String pagingImg = BoardPagination.pagingStr(
				totalCount, pageSize, blockPage, pageNum, "../qna-board/list.do",
		...
		req.getRequestDispatcher("/html/qna-board-list.jsp").forward(req, resp);
	}
}
